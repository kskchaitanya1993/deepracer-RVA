...
def reward_function(params):
    ...
    def closest_indexes(racing_xy, car_xy):
        ...
        sec_closest_racing_index = distance_wo_closest.index(
            min(distance_wo_closest))
    ...
    def dist_to_racing_line(closest_coor, second_closest_coor, car_coor):
        ...
        base = dist_bw_points(
            closest_coor[0], second_closest_coor[0], closest_coor[1], second_closest_coor[1])

        # Distances between car and closest and second closest racing point
        side1 = dist_bw_points(
            car_coor[0], closest_coor[0], car_coor[1], closest_coor[1])
        side2 = dist_bw_points(
            car_coor[0], second_closest_coor[0], car_coor[1], second_closest_coor[1])
        ...
        try:
            distance = (0.5/base) * (side1 + base + side2)**0.5 * (-side1 + base +
                                                                   side2)**0.5 * (side1 - base + side2)**0.5 * (side1 + base - side2)**0.5
        except:
            distance = side1
    ...
    try:
        reward += (((5 * progress) + (4 * speed) + (2 * reward_obj.acceleration(params)))
                   / ((5 * time_spent_till_now) + (4 * dist) + (3 * direction_diff) + (1 * distance_from_center)))
    except:
        reward += 1e-6

    reward += 5 * (1 - time_spent_till_now/15)
